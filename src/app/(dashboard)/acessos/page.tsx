...
import { Plus, Search, MoreVertical, ShieldAlert, X, AlertTriangle, CheckCircle } from 'lucide-react'
import { GrupoAcessoResponseDTO } from '@/types/api'
import { api } from '@/services/api'

export default function AcessosPage() {
  const [grupos, setGrupos] = useState<GrupoAcessoResponseDTO[]>([])
  ...
  const [editingItem, setEditingItem] = useState<GrupoAcessoResponseDTO | null>(null)
  const [deletingItem, setDeletingItem] = useState<GrupoAcessoResponseDTO | null>(null)
  ...
  const fetchGruposAcesso = async () => {
    try {
      const response = await api.get('/grupos-acesso')
  ...
  const handleCreate = async () => {
    ...
    try {
      // Importante: No Java, o GrupoAcessoRequestDTO pode exigir um array de permissões
      const response = await api.post('/grupos-acesso', {
        ...newItem,
        permissoes: [] // Garantindo um array vazio caso o backend não aceite nulo
      })
      ...
      showNotification("Falha ao criar o grupo. Verifique o console.", "error")
    }
  }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await api.put(`/grupos-acesso/${editingItem.id}`, {
        nome: editingItem.nome,
        descricao: editingItem.descricao
      })
      setGrupos(prev => prev.map(g => g.id === editingItem.id ? editingItem : g))
      setEditingItem(null)
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/grupos-acesso/${deletingItem.id}`)
      setGrupos(prev => prev.filter(g => g.id !== deletingItem.id))
      setDeletingItem(null)
      setDeleteInput('')
      showNotification("Grupo excluído com sucesso!", "success")
    } catch (error) {
      console.error("Erro ao excluir grupo de acesso:", error)
      showNotification("Falha ao excluir o grupo.", "error")
    }
  }
    ...
    <div className="relative min-h-[calc(100vh-4rem)] w-full overflow-hidden text-zinc-100">
      ...
      <div className="relative z-10 space-y-8 pb-8 px-6 pt-6 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-zinc-800/50">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">Gerencie os grupos de acesso e permissões de usuários.</p>
          </div>
          <button 
          ...
          <div className="overflow-x-auto min-h-[300px]">
            <table className="w-full text-left text-sm">
              <thead className="bg-zinc-900/40 border-b border-zinc-800/60">
                <tr>
                  <th className="px-6 py-4 font-medium text-zinc-400">Grupo</th>
                  <th className="px-6 py-4 font-medium text-zinc-400">Descrição</th>
                  <th className="px-6 py-4 font-medium text-zinc-400">Permissões</th>
                  <th className="px-6 py-4 font-medium text-zinc-400 text-right">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-800/50">
                {isLoading ? (
                  <tr><td colSpan={4} className="px-6 py-12 text-center text-zinc-500 animate-pulse font-mono text-xs uppercase tracking-widest">Carregando acessos...</td></tr>
                ) : filteredGrupos.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-12 text-center text-zinc-500 font-mono text-xs uppercase tracking-widest">Nenhum grupo encontrado.</td></tr>
                ) : (
                  filteredGrupos.map((grupo) => (
                    <tr key={grupo.id} className="hover:bg-zinc-900/30 transition-colors">
                      <td className="px-6 py-4 font-medium text-zinc-200">{grupo.nome}</td>
                      <td className="px-6 py-4 text-zinc-400">{grupo.descricao || '-'}</td>
                      <td className="px-6 py-4">
                        <span className="px-2 py-1 rounded bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-mono">{grupo.permissoes?.length || 0} Regras</span>
                      </td>
                      <td className="px-6 py-4 text-right relative">
                        <button onClick={() => setOpenMenuId(openMenuId === grupo.id ? null : grupo.id)} className="p-2 hover:bg-zinc-800 rounded-md text-zinc-400 hover:text-white transition-colors relative z-10">
                        ...
                        {openMenuId === grupo.id && (
                          <>
                            ...
                            <div className="absolute right-6 top-10 mt-1 w-36 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                              <button onClick={() => { setEditingItem(grupo); setOpenMenuId(null); }} className="w-full text-left px-4 py-2.5 text-sm text-zinc-300 hover:bg-zinc-800 hover:text-white transition-colors">Editar</button>
                              <div className="h-px bg-zinc-800/80 w-full" />
                              <button onClick={() => { setDeletingItem(grupo); setOpenMenuId(null); setDeleteInput(''); }} className="w-full text-left px-4 py-2.5 text-sm text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-colors">Excluir</button>
                            </div>
                          </>
                        )}
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-zinc-800/80">
              <h3 className="text-lg font-semibold text-white">Novo Grupo de Acesso</h3>
              <button onClick={() => setIsCreating(false)} className="text-zinc-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">Nome do Grupo</label>
                <input 
                  type="text" 
                  value={newItem.nome} 
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-white focus:border-orange-500 focus:outline-none min-h-[80px]" 
                  placeholder="Descreva os acessos deste grupo..." 
                />
              </div>
            </div>
            <div className="p-4 border-t border-zinc-800/80 flex justify-end gap-3 bg-zinc-950/50">
              ...
              <button onClick={handleCreate} className="px-4 py-2 bg-orange-600 hover:bg-orange-500 text-white text-sm font-medium rounded-lg transition-colors shadow-lg shadow-orange-500/20">Criar Grupo</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-zinc-800/80">
              <h3 className="text-lg font-semibold text-white">Editar Grupo de Acesso</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-red-900/30 rounded-xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-semibold text-white mb-2">Excluir Grupo "{deletingItem.nome}"?</h3>
              <p className="text-sm text-zinc-400 mb-6">Usuários atrelados a este grupo perderão os seus acessos. Digite o nome do grupo abaixo para confirmar.</p>
              ...
              <div className="flex gap-3">
                ...
                <button disabled={deleteInput !== deletingItem.nome} onClick={handleDelete} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-500 disabled:bg-red-600/30 disabled:text-white/50 text-white text-sm font-medium rounded-lg transition-colors">Confirmar Exclusão</button>
              </div>
            </div>
          </div>
        </div>
